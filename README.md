...
**Teams are entirely admin-assigned.** There's no drafting or self-picking — you build each team's roster (who's on it) on the Setup page ahead of time. Players pick their own name from a dropdown of everyone on the roster who hasn't joined yet, so there's no typo-prone free-text entry and no way to end up unplaced; they're put on their team the instant they pick their name. If someone grabs the wrong name by mistake, "Not you? Switch player" on their waiting-room screen frees that name back up and sends them back to the dropdown (only works before the hunt starts).
...
If you've already written out your full location list in a spreadsheet, `npm run seed-locations -- /path/to/your.csv` loads it in one shot instead of typing everything into the Setup page by hand. The CSV needs a header row with columns `Name, Acceptable Answers, Clue, Task, Extra Hint, Team, Order` — `Acceptable Answers` is pipe-separated (`CN Tower|CN|Canada's National Tower`), `Team` is `1`, `2`, or `BOTH` for a stop shared by both teams' routes (or just use two rows, one per team), and `Order` is that team's visiting order for the row (1, 2, 3, ...) — the two teams can visit a shared stop at completely different points in their own routes. Three more columns — `Clue (English)`, `Task (English)`, `Extra Hint (English)` — are optional and feed the players' slang/English toggle (see below); leave them out entirely, or blank per-row, and that content just won't have an English version. It always requires the two teams to already exist (`npm run setup`) and replaces every location, so only run it once, before you've hand-edited anything on the Setup page you don't want overwritten.

## The slang/English toggle

Every player screen has a small "🔤 Translate to English" button. The app's own built-in copy (headings, buttons, instructions) flips to a deliberately overly-formal, overly-polite English translation of the Toronto slang — think the "4 Y'all Entertainment" style of translating slang to stiff, buttoned-up English. Each location's clue, task, and extra hint can have their own English version too — set them on the Setup page (or via the CSV columns above) — and if a location doesn't have one, it just stays in slang when the toggle is on rather than showing blank. The toggle preference is remembered per-browser (`localStorage`) and switching it never triggers a network request — it just re-renders whatever's already on screen.
